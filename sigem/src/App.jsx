    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col">
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          {/* Top Navigation */}
          <nav className="flex gap-2 mx-4 overflow-x-auto">
            <button
              onClick={() => setActiveTab('documents')}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-sm font-medium whitespace-nowrap ${
                activeTab === 'documents'
                ? 'bg-indigo-900/40 text-indigo-300 shadow-sm border border-indigo-900'
                : 'text-slate-500 hover:bg-slate-800 hover:text-slate-100 border border-transparent'
              }`}
            >
          ...
          {/* Status Badge */}
          <div className="hidden lg:flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-full border border-slate-700 shrink-0">
            {dbReady ? (
              <>
                <ShieldCheck className="w-4 h-4 text-emerald-500" />
                <span className="text-xs font-medium text-slate-400">Conexión Local Segura</span>
              </>
            ) : (
              <>
                <Database className="w-4 h-4 text-amber-500 animate-pulse" />
                <span className="text-xs font-medium text-slate-400">Inicializando motor...</span>
              </>
            )}
          </div>
        </div>
